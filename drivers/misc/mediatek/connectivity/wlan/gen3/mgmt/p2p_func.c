			...
			UINT_8 ucAction)
{
	...
	case WLAN_PA_20_40_BSS_COEX:
		DBGLOG(P2P, TRACE, "Found WLAN_PA_20_40_BSS_COEX, SA: %pM - DA: %pM, SeqNo: %d\n",
			prActFrame->aucSrcAddr,
			prActFrame->aucDestAddr,
			prMgmtTxMsdu->ucTxSeqNum);
		break;
	...
	case WLAN_PA_GAS_INITIAL_REQ:
		DBGLOG(P2P, TRACE, "Found WLAN_PA_GAS_INITIAL_REQ, SA: %pM - DA: %pM, SeqNo: %d\n",
			prActFrame->aucSrcAddr,
			prActFrame->aucDestAddr,
			prMgmtTxMsdu->ucTxSeqNum);
		break;
	case WLAN_PA_GAS_INITIAL_RESP:
		DBGLOG(P2P, TRACE, "Found WLAN_PA_GAS_INITIAL_RESP, SA: %pM - DA: %pM, SeqNo: %d\n",
			prActFrame->aucSrcAddr,
			prActFrame->aucDestAddr,
			prMgmtTxMsdu->ucTxSeqNum);
		break;
	case WLAN_PA_GAS_COMEBACK_REQ:
		DBGLOG(P2P, TRACE, "Found WLAN_PA_GAS_COMEBACK_REQ, SA: %pM - DA: %pM, SeqNo: %d\n",
			prActFrame->aucSrcAddr,
			prActFrame->aucDestAddr,
			prMgmtTxMsdu->ucTxSeqNum);
		break;
	case WLAN_PA_GAS_COMEBACK_RESP:
		DBGLOG(P2P, TRACE, "Found WLAN_PA_GAS_COMEBACK_RESP, SA: %pM - DA: %pM, SeqNo: %d\n",
			prActFrame->aucSrcAddr,
			prActFrame->aucDestAddr,
			prMgmtTxMsdu->ucTxSeqNum);
		break;
	case WLAN_TDLS_DISCOVERY_RESPONSE:
		DBGLOG(P2P, TRACE, "Found WLAN_TDLS_DISCOVERY_RESPONSE, SA: %pM - DA: %pM, SeqNo: %d\n",
			prActFrame->aucSrcAddr,
			prActFrame->aucDestAddr,
			prMgmtTxMsdu->ucTxSeqNum);
		break;
	default:
		DBGLOG(P2P, TRACE, "Unknown action: 0x%x, SA: %pM - DA: %pM, SeqNo: %d\n",
			ucAction,
			prActFrame->aucSrcAddr,
			prActFrame->aucDestAddr,
			prMgmtTxMsdu->ucTxSeqNum);
			...
			IN UINT_8 ucCategory)
{
	...
	case WLAN_ACTION_SPECTRUM_MGMT:
		DBGLOG(P2P, TRACE, "Found WLAN_ACTION_SPECTRUM_MGMT, SA: %pM - DA: %pM, SeqNO: %d\n",
			prActFrame->aucSrcAddr,
			prActFrame->aucDestAddr,
			prMgmtTxMsdu->ucTxSeqNum);
		break;
	case WLAN_ACTION_QOS:
		DBGLOG(P2P, TRACE, "Found WLAN_ACTION_QOS, SA: %pM - DA: %pM, SeqNO: %d\n",
			prActFrame->aucSrcAddr,
			prActFrame->aucDestAddr,
			prMgmtTxMsdu->ucTxSeqNum);
		break;
	case WLAN_ACTION_DLS:
		DBGLOG(P2P, TRACE, "Found WLAN_ACTION_DLS, SA: %pM - DA: %pM, SeqNO: %d\n",
			prActFrame->aucSrcAddr,
			prActFrame->aucDestAddr,
			prMgmtTxMsdu->ucTxSeqNum);
		break;
	case WLAN_ACTION_BLOCK_ACK:
		DBGLOG(P2P, TRACE, "Found WLAN_ACTION_BLOCK_ACK, SA: %pM - DA: %pM, SeqNO: %d\n",
			prActFrame->aucSrcAddr,
			prActFrame->aucDestAddr,
			prMgmtTxMsdu->ucTxSeqNum);
		break;
	case WLAN_ACTION_PUBLIC:
		DBGLOG(P2P, TRACE, "Found WLAN_ACTION_PUBLIC, SA: %pM - DA: %pM, SeqNO: %d\n",
			prActFrame->aucSrcAddr,
			prActFrame->aucDestAddr,
			prMgmtTxMsdu->ucTxSeqNum);
	...
	case WLAN_ACTION_RADIO_MEASUREMENT:
		DBGLOG(P2P, TRACE, "Found WLAN_ACTION_RADIO_MEASUREMENT, SA: %pM - DA: %pM, SeqNO: %d\n",
			prActFrame->aucSrcAddr,
			prActFrame->aucDestAddr,
			prMgmtTxMsdu->ucTxSeqNum);
		break;
	case WLAN_ACTION_FT:
		DBGLOG(P2P, TRACE, "Found WLAN_ACTION_FT, SA: %pM - DA: %pM, SeqNO: %d\n",
			prActFrame->aucSrcAddr,
			prActFrame->aucDestAddr,
			prMgmtTxMsdu->ucTxSeqNum);
		break;
	case WLAN_ACTION_HT:
		DBGLOG(P2P, TRACE, "Found WLAN_ACTION_HT, SA: %pM - DA: %pM, SeqNO: %d\n",
			prActFrame->aucSrcAddr,
			prActFrame->aucDestAddr,
			prMgmtTxMsdu->ucTxSeqNum);
		break;
	case WLAN_ACTION_SA_QUERY:
		DBGLOG(P2P, TRACE, "Found WLAN_ACTION_SA_QUERY, SA: %pM - DA: %pM, SeqNO: %d\n",
			prActFrame->aucSrcAddr,
			prActFrame->aucDestAddr,
			prMgmtTxMsdu->ucTxSeqNum);
		break;
	case WLAN_ACTION_PROTECTED_DUAL:
		DBGLOG(P2P, TRACE, "Found WLAN_ACTION_PROTECTED_DUAL, SA: %pM - DA: %pM, SeqNO: %d\n",
			prActFrame->aucSrcAddr,
			prActFrame->aucDestAddr,
			prMgmtTxMsdu->ucTxSeqNum);
		break;
	case WLAN_ACTION_WNM:
		DBGLOG(P2P, TRACE, "Found WLAN_ACTION_WNM, SA: %pM - DA: %pM, SeqNO: %d\n",
			prActFrame->aucSrcAddr,
			prActFrame->aucDestAddr,
			prMgmtTxMsdu->ucTxSeqNum);
		break;
	case WLAN_ACTION_UNPROTECTED_WNM:
		DBGLOG(P2P, TRACE, "Found WLAN_ACTION_UNPROTECTED_WNM, SA: %pM - DA: %pM, SeqNO: %d\n",
			prActFrame->aucSrcAddr,
			prActFrame->aucDestAddr,
			prMgmtTxMsdu->ucTxSeqNum);
		break;
	case WLAN_ACTION_TDLS:
		DBGLOG(P2P, TRACE, "Found WLAN_ACTION_TDLS, SA: %pM - DA: %pM, SeqNO: %d\n",
			prActFrame->aucSrcAddr,
			prActFrame->aucDestAddr,
			prMgmtTxMsdu->ucTxSeqNum);
		break;
	case WLAN_ACTION_SELF_PROTECTED:
		DBGLOG(P2P, TRACE, "Found WLAN_ACTION_SELF_PROTECTED, SA: %pM - DA: %pM, SeqNO: %d\n",
			prActFrame->aucSrcAddr,
			prActFrame->aucDestAddr,
			prMgmtTxMsdu->ucTxSeqNum);
		break;
	case WLAN_ACTION_WMM: /* WMM Specification 1.1 */
		DBGLOG(P2P, TRACE, "Found WLAN_ACTION_WMM, SA: %pM - DA: %pM, SeqNO: %d\n",
			prActFrame->aucSrcAddr,
			prActFrame->aucDestAddr,
			prMgmtTxMsdu->ucTxSeqNum);
		break;
	case WLAN_ACTION_VENDOR_SPECIFIC:
		DBGLOG(P2P, TRACE, "Found WLAN_ACTION_VENDOR_SPECIFIC, SA: %pM - DA: %pM, SeqNO: %d\n",
			prActFrame->aucSrcAddr,
			prActFrame->aucDestAddr,
			prMgmtTxMsdu->ucTxSeqNum);
		break;
	default:
		DBGLOG(P2P, TRACE, "Unknown Category: 0x%x, SA: %pM - DA: %pM\n",
			ucCategory,
			prActFrame->aucSrcAddr,
			prActFrame->aucDestAddr);
	}
