...
int mtk_p2p_cfg80211_scan(struct wiphy *wiphy, struct cfg80211_scan_request *request)
{
	...
	do {
		...
		DBGLOG(P2P, INFO, "mtk_p2p_cfg80211_scan Requesting channel number:%d\n",
							request->n_channels);
				       ...
				       unsigned int duration, u64 *cookie)
{
	...
	do {
		...
		*cookie = prGlueP2pInfo->u8Cookie++;
		...
		DBGLOG(P2P, TRACE, "Cookie: 0x%llx, duration: %d\n", *cookie, duration);
					      ...
					      u64 cookie)
{
	...
	do {
		...
		DBGLOG(P2P, TRACE, "P2P: cancel remain on channel 0x%llx\n", cookie);
				...
				u64 *cookie)
{
	...
	do {
		...
		DBGLOG(P2P, INFO, "mtk_p2p_cfg80211_mgmt_tx Off channel %d ext listen wait %d\n",
