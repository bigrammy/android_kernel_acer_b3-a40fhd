	...
	unsigned int page, int status, void *userdata)
{
	...
	if (status)
		pr_err("op_type:%d, status:%d, block:%d, page:%d\n", *op, status, block, page);
...
int mtk_nand_ftl_compare_buf(char *buf0, char *buf1, int len)
{
	...
	if (len <= 0)
		pr_err("mtk_nand_ftl_compare_buf, len <= 0!!\n");
		...
		mtk_nand_callback_func func)
{
	...
	ret = mtk_nand_chip_write_page(info, w_page_buf, w_oob_buf, block, page,
		more, func, &op_write);
	if (ret)
		pr_err("mtk_nand_chip_write_page return fail:%d\n", ret);
}
		...
		bool print, bool compare)
{
	...
	if (ret) {
		pr_err("mtk_nand_chip_read_page return fail:%d\n", ret);
		return ret;
	} else if (print) {
...
OUT:
	if (ret)
		pr_err("read compare fail: block:%d, page:%d\n", block, page);
	return ret;
}
	...
	size_t count, loff_t *data)
{
	...
	if (!strcmp(cmd, "init")) {
		...
		if (ret)
			pr_err("stop mntl block thread fail, ret:%d\n", ret);
		else
			pr_err("stop mntl block thread success\n");
	} else if (!strcmp(cmd, "wakebld")) {
		ret = mntl_block_thread_wakeup();
		if (ret)
			pr_err("wakeup mntl block thread fail, ret:%d\n", ret);
		else
			pr_err("wakeup mntl block thread success\n");
	} else if (!strcmp(cmd, "erase")) {
		if (sscanf(pbuf, "%d%d", &block, &more) != 2)
			goto INVALID_ARG;
		ret = mtk_nand_chip_erase_block(info, block, more, func, &op_erase);
		if (ret)
			pr_err("mtk_nand_chip_erase_block return fail:%d\n", ret);
		...
		for (i = 0; i < pages; i++) {
			...
			if (ret) {
				pr_err("read or compare fail:block:%d, page:%d\n", block, page);
				break;
			}
		...
		if (i == pages)
			pr_err("All pages readback and compare pass ^^\n");
		else
			pr_err("Some page readback or compare fail ==!!^\n");
		...
		pr_err("rowAddr:0x%x, block:%d, page:%d\n", rowAddr, block, page);
