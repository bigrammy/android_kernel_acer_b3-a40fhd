...
#define MTP_RESPONSE_DEVICE_CANCEL  0x201F //Neostra mohx add for bug147513 20170509
...
struct mtp_dev {
	...
	int xfer_result;
    int dev_disconnected; //Neostra mohx add for bug147513 20170509
};
...
static void mtp_ueventToDisconnect(struct mtp_dev *dev);//Neostra mohx add for bug147513 20170509
...
static void receive_file_work(struct work_struct *data)
{
	...
	while (count > 0 || write_req) {
		if (count > 0) {
			...
			if (ret < 0) {
				r = -EIO;
                //Neostra mohx add for bug147513 20170509
				//dev->state = STATE_ERROR;
...
mtp_function_unbind(struct usb_configuration *c, struct usb_function *f)
{
	...
	dev->state = STATE_OFFLINE;
    dev->dev_disconnected = 1;//Neostra mohx add for bug147513 20170509
		...
		unsigned intf, unsigned alt)
{
	...
	dev->state = STATE_READY;
    dev->dev_disconnected = 0;//Neostra mohx add for bug147513 20170509
