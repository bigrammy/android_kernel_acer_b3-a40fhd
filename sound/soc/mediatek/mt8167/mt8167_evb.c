...
//Neostra huangjianlong 20170524
static void mt8167_evb_dev_shutdown(struct platform_device *pdev)
{
	int ret = 0;
	struct snd_soc_card *card = &mt8167_evb_card;
	struct mt8167_evb_priv *card_data = snd_soc_card_get_drvdata(card);

	printk("[huangjianlong] mt8167_evb_dev_shutdown: disable audio PA ..... \n");
	if (IS_ERR(card_data->pin_states[PIN_STATE_EXTAMP_OFF]))
		return;
	...
	if (ret)
		dev_err(card->dev, "%s failed to select state %d\n",
			__func__, ret);
	printk("[huangjianlong] mt8167_evb_dev_shutdown: disable audio PA 222..... \n");
/*	printk("[huangjianlong] mt8167_evb_dev_shutdown  wait some time..... \n");
	if (card_data->ext_spk_amp_shutdown_time_us > 0)
		usleep_range(card_data->ext_spk_amp_shutdown_time_us,
			card_data->ext_spk_amp_shutdown_time_us + 1);
*/
...
static struct platform_driver mt8167_evb_mach_driver = {
	...
	.shutdown = mt8167_evb_dev_shutdown, //Neostra huangjianlong 20170524
};
